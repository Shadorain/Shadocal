    ...
    match &*accounts.read_unchecked() {
        Some(Ok(accounts)) => rsx! {
            div {
                ...
                div { class: "hover:bg-gray-100 dark:hover:bg-ghdarkmetal rounded w-1/4 py-1",
                    div { class: "grid grid-cols-[auto,1fr,auto] items-center gap-2 px-1",
                        ...
                        div { class: "leading-snug text-xs text-left",
                            ...
                            //     "Using  Version"
                            // }
                            p { class: "font-medium text-gray-700 dark:text-gray-100", "Selected account" }
                        }
        ...
        Some(Err(err)) => {
            println!("Err : {err}");
            Err(dioxus::CapturedError::from_display(
